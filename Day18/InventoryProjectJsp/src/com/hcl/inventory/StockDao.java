...
public class StockDao {
	PreparedStatement pst;
	  ...
	  public int generateorderid() {
		  ...
		  int orderid = 0;
		  String cmd = "Select case when Max(orderid) is Null then 1 else Max(orderid) + 1 End orderNo from orders";
		  try {
	  ...
	  public String placeOrderDao(String stockid, int qnty) {
		...
		try {
			if (stock != null) {
				...
				// fetch quantity of respective stockid from stock table\
				
				  if (qnty != 0 && qnty <= tempQnty) {
					  double price = stock.getPrice();
					  double billAmt = qnty * price;
					  String cmd  = "INSERT INTO Orders(OrderId,StockId,QtyOrd,billAmt) values(?,?,?,?);";
					  pst = con.prepareStatement(cmd);
